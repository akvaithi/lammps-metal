...
int DeviceT::compile_kernels() {
  ...
  #if !(defined(USE_OPENCL) || defined(USE_HIP))
  std::cerr << "ptx_arch: " << _ptx_arch << ", gpu->arch: " << gpu->arch() << std::endl;
  // if (_ptx_arch>gpu->arch() || floor(_ptx_arch)<floor(gpu->arch()))
  //  return -4;
  #endif
  std::cerr << "Passed ptx_arch check" << std::endl;

  _config_id=gpu_lib_data[1];
  std::cerr << "Config id set" << std::endl;
  ...
  std::cerr << "Simd size set" << std::endl;
  ...
  if (_block_pair == -1) {
    ...
    // Fallbacks for Metal/stub where gpu_lib_data might be all 0s
    if (_block_pair == 0) _block_pair = 256;
    if (_block_bio_pair == 0) _block_bio_pair = 256;
    if (_block_ellipse == 0) _block_ellipse = 256;
  } else {
    _block_bio_pair=_block_pair;
    _block_ellipse=_block_pair;
  }
  _pppm_block=gpu_lib_data[12];
  if (_pppm_block == 0) _pppm_block = 256;
  
  _block_nbor_build=gpu_lib_data[13];
  if (_block_nbor_build == 0) _block_nbor_build = 256;
  
  _block_cell_2d=gpu_lib_data[14];
  if (_block_cell_2d == 0) _block_cell_2d = 64;
  
  _block_cell_id=gpu_lib_data[15];
  if (_block_cell_id == 0) _block_cell_id = 64;

  _max_shared_types=gpu_lib_data[16];
  if (_max_shared_types == 0) _max_shared_types = 11;
  
  _max_bio_shared_types=gpu_lib_data[17];
  if (_max_bio_shared_types == 0) _max_bio_shared_types = 11;
  
  _pppm_max_spline=gpu_lib_data[18];
  std::cerr << "Finished assigning variables" << std::endl;
  ...
  if (static_cast<size_t>(_block_pair) > gpu->group_size_dim(0) ||
    ...
    std::cerr << "Invalid configuration ... returning -13" << std::endl;
    return -13;
  }
  std::cerr << "Passed configuration check" << std::endl;
