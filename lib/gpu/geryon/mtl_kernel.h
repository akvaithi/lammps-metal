...
class UCL_Kernel {
 public:
  ...
  inline void set_arg(const unsigned index, const dtype * const arg) {
    if (index==_num_args)
      ...
      _kernel_args[index]=const_cast<dtype*>(arg);
      _arg_sizes[index] = sizeof(dtype);
      _arg_is_device_ptr[index] = false;
    } else
      assert(0==1); // Must add kernel parameters in sequential order
  }

  inline void set_arg(const unsigned index, const CUdeviceptr * const arg) {
    if (index==_num_args)
      add_arg(arg);
    else if (index<_num_args) {
      _kernel_args[index]=(void *)arg;
      _arg_sizes[index] = sizeof(CUdeviceptr);
      _arg_is_device_ptr[index] = true;
    } else
      assert(0==1); // Must add kernel parameters in sequential order
  }
  ...
  inline void add_arg(const CUdeviceptr* const arg) {
    ...
    _arg_sizes[_num_args] = sizeof(CUdeviceptr);
    _arg_is_device_ptr[_num_args] = true;
    _num_args++;
  ...
  inline void add_arg(const dtype* const arg) {
    ...
    _arg_sizes[_num_args] = sizeof(dtype);
    _arg_is_device_ptr[_num_args] = false;
    _num_args++;
    if (_num_args>UCL_MAX_KERNEL_ARGS) assert(0==1);
  }
  ...
  inline void add_arg(const UCL_D_Vec<numtyp> * const arg)
    { add_arg((const CUdeviceptr*)&arg->cbegin()); }
  ...
  inline void add_arg(const UCL_Vector<hosttype, devtype> * const arg)
    { add_arg((const CUdeviceptr*)&arg->device.cbegin()); }
  ...
  inline void run() {
    CU_SAFE_CALL(cuLaunchKernel(_kernel,_num_blocks[0],_num_blocks[1],
                                ...
                                _arg_sizes, _arg_is_device_ptr, _num_args));
  }
 ...
 private:
  ...
  size_t _arg_sizes[UCL_MAX_KERNEL_ARGS];
  bool _arg_is_device_ptr[UCL_MAX_KERNEL_ARGS];
};
