...
// --------------------------------------------------------------------------
template <class mat_type, class host_type>
inline void _mtl_host_view(mat_type &mat, host_type &in, const size_t offset,
                           const size_t numsize) {
  void **ptr=(void **)&mat.host_ptr();
  *ptr=(char*)in+offset*numsize;
}
...
inline void _mtl_set_2D_loc(CUDA_MEMCPY2D &ins, const size_t dpitch,
...
template <int mem> struct _mtl_set_2D_mem;
template <> struct _mtl_set_2D_mem<1>
  { static CUmemorytype a() { return CU_MEMORYTYPE_HOST; } };
template <> struct _mtl_set_2D_mem<2>
...
template<> struct _ucl_memcpy<2,2> {
  ...
  template <class p1, class p2>
      static inline void mc(p1 &dst, const size_t dpitch, const p2 &src,
                            ...
                            const size_t rows) {
    CUDA_MEMCPY2D ins;
    _mtl_set_2D_loc(ins,dpitch,spitch,cols,rows);
    ins.dstMemoryType=_mtl_set_2D_mem<p1::MEM_TYPE>::a();
    ins.srcMemoryType=_mtl_set_2D_mem<p2::MEM_TYPE>::a();
